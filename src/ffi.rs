use crate::media_core::video_rewrap::{
    ...
    nearest_keyframe, parse_ffprobe_keyframes, parse_ffprobe_metadata, parse_project_json,
    parse_timecode_to_ms, rewrap_preflight, rewrap_report_csv, rewrap_report_txt,
    temp_segment_path, validate_segments, validated_container_extension, ExportMode, RewrapProject,
...
mod tests {
    ...
    #[test]
    fn ffi_rewrap_preflight_passes_for_any_output_container() {
        ...
        assert_eq!(parsed["ok"], true);
        assert!(parsed["preflight"]["guidance"]
            .as_array()
            .unwrap()
            .iter()
            .any(|g| g.as_str().unwrap().contains("stream-copy")));
    }
    ...
    fn ffi_export_plan_segment_extension_follows_output_suffix() {
        ...
        assert!(parsed_mxf["error"]
            .as_str()
            .unwrap()
            .contains("Unsupported output container"));
    }
